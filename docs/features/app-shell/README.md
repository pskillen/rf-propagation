...
- **Persistent chrome** — the Station bar and Conditions bar, reserved as empty slots on `AppChrome` in this phase, filled by phases 6 and 7. Always visible, on every surface, at every breakpoint.
...
- **`useViewerUrlState` is unused so far.** The hook exists (`src/app/hooks/useViewerUrlState.ts`) but no component calls it yet — `surface` navigation in this phase is entirely pathname-driven (`react-router-dom`'s routes), not query-string-driven. It is infrastructure for phase 6/7's fields, which will need the `useSearchParams()` wiring. See [url-state-codec.md](url-state-codec.md) for the reasoning.
