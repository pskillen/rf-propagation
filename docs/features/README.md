...
| Explore | [explore/README.md](explore/README.md) | Shipped — labelled vertical cross-section, an illustration ray overlay (operator-sized controls, filter/colour-by/layer-solo, rendered on both the cross-section and the globe from one engine call), in-place term definitions, and the reusable "explain this" entry point plus a reconciling link-budget breakdown panel; Path/Compare UI (phases 12–13) not yet built |
