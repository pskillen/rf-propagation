...
| Station | [station/README.md](station/README.md) | In progress — Station model, fail-soft persistence, URL codec registration, QTH picker, antenna model with absolute dBi, TX power/noise environment inputs, and the always-populated Station bar are shipped; the propagation engine isn't wired to it yet (phase 8) |
| Conditions | [conditions/README.md](conditions/README.md) | In progress — Conditions model (SFI/Kp unit contract), now-toggle + time scrubber, URL codec registration, NOAA SWPC live fetch with a full live/last-known/manual/preset fallback chain and always-visible provenance, and the trimmed amateur-HF band catalogue + chips are shipped; the propagation engine isn't wired to it yet (phase 8) |
