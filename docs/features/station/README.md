...
| Antenna editing in place | Shipped (`feat/antenna-inline-edit`, superseding `fix/reach-directionality-antenna-greyline`'s "Edit" button) | Selecting an antenna's pill shows its editable form immediately; Save replaces it in place instead of requiring a near-duplicate; an "Unsaved changes" indicator and a switch-while-dirty confirm guard replace silent discard; heading field also exposed for dipoles, not just beams — [antenna-model.md](antenna-model.md) |
